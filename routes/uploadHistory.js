    ...
    async (req, res, next) => {
        try {
            const userFound = await User.findById(req.userInfo.userId)
                .populate(
                    'uploadedFiles.blockId',
                    'currentInfo uploadedToNetwork'
                )
                .populate('uploadedFiles.relatedVideo', 'title')
                .populate({
            ...
            const uploadedVideosRaw = await Promise.all(
            ...
            for (
                let fileCount = 0;
                fileCount < userFound.uploadedFiles.length;
                fileCount++
            ) {
                file = {};
                file = await userFound.uploadedFiles[fileCount].subPopulate(
                    'fileId'
                );
