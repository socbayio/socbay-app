    ...
    if (tagFound) {
        ...
        for (let videoCount = 0; videoCount< tagFound.videos.length; videoCount++){
            try {
                ...
                if (video) {videoArray.push(video);} 
...
const getVideosFromTagPromiseStyleOld = async (tagName) => {
    const tagFound = await videoTag.findOne({tagName: tagName});
    if (tagFound){
        const promises = tagFound.videos.map(videoId => Video.findById(videoId));
        const results = await Promise.allSettled(promises)
        const videoArray = results.filter(p => (p.status === 'fulfilled')).map(v=>v.value).filter(x => x !== null);
        return {name: tagName, videos: videoArray};
    }
    return;
}
...
const getVideosFromTagPromiseStyle = async (tagName) => {
    const tagFound = await videoTag.findOne({tagName: tagName}).populate('videos.videoId');
    if (tagFound){
        videoArray = [];
        for (let videoCount = 0; videoCount < tagFound.videos.length; videoCount++){
...
const getVideosChannelOld = async (channelId) => {
    ...
    if (userFound) {
        const promises = userFound.uploadedVideos.map(videoId => Video.findById(videoId));
        const results = await Promise.allSettled(promises);
        const videoArray = results.filter(p => (p.status === 'fulfilled')).map(v=>v.value).filter(x => x !== null);
        channelInfo = {
...
const getVideosChannel = async (channelId) => {
    const userFound = await User.findById(channelId).populate('uploadedVideos.videoId');
...
const pushVideoToTag = async (tagName, videoId)=>{
    const videoTagFound = await videoTag.findOneAndUpdate({tagName:tagName},{$push: {videos: {videoId: videoId}}});
    if (!videoTagFound){
